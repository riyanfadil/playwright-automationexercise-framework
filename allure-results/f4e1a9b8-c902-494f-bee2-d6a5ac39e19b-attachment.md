...
Call log:
  - waiting for locator('button.close-modal') to be visible
    22 × locator resolved to hidden <button data-dismiss="modal" class="btn btn-success close-modal btn-block">Continue Shopping</button>
...
```ts
  ...
  16 |     
  17 |     // Target the visible buttons in the product cards directly
  18 |     this.firstProductAddToCartBtn = page.locator('.productinfo .add-to-cart').nth(0);
  19 |     this.secondProductAddToCartBtn = page.locator('.productinfo .add-to-cart').nth(1); 
  20 |     
  ...
  32 |     await this.firstProductAddToCartBtn.click({ force: true });
> 33 |     await this.continueShoppingBtn.waitFor({ state: 'visible', timeout: 10000 });
     |                                    ^ TimeoutError: locator.waitFor: Timeout 10000ms exceeded.
  34 |   }
  35 | 
  36 |   async addSecondProductToCart() {
  37 |     await this.secondProductAddToCartBtn.click({ force: true });
  38 |     await this.viewCartLinkInModal.waitFor({ state: 'visible', timeout: 10000 });
  39 |   }
  40 | 
  41 |   async clickContinueShopping() {
  42 |     await this.continueShoppingBtn.click();
  43 |     // Wait for the modal to hide
  44 |     await this.continueShoppingBtn.waitFor({ state: 'hidden', timeout: 10000 });
  45 |   }
  46 | 
  47 |   async clickViewCartFromModal() {
  48 |     await this.viewCartLinkInModal.click();
  49 |   }
  50 | }
  51 | 
```